...
def action(act):
   ...
   try:
      ...
      if isinstance(sm, float):
          my_text_var.set("{0} {1} {2} = {3:.3}".format(a.get(),funcions_list[act][1],b.get(),sm))
      else:
          my_text_var.set("{0} {1} {2} = {3}".format(a.get(),funcions_list[act][1],b.get(),sm))
   except ValueError:
      my_text_var.set("Wrong inputs")

win=Tk()
win.title("Calculater TKinter");
win.geometry("300x165")
#window.iconbitmap('@c.xbm')  #Linux Only
my_text_var = StringVar(value = "Hello! Please Insert Values")
...
label=Label(win, textvariable = my_text_var, font=('Calibri 15'))
